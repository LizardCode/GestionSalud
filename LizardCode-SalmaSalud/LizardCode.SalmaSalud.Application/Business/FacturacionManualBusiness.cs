...
{
    public class FacturacionManualBusiness: BaseBusiness, IFacturacionManualBusiness
    {
        ...
        public FacturacionManualBusiness(
        ...
        public async Task<FacturacionManualViewModel> Get(int idComprobanteVta)
        {
            ...
            var model = _mapper.Map<FacturacionManualViewModel>(comprobante);
            model.Items = _mapper.Map<List<FacturacionManualDetalle>>(items);
        ...
        public async Task<DataTablesResponse<Custom.ComprobanteVenta>> GetAll(DataTablesRequest request)
        {
            ...
            builder.Append($"AND IdTipoComprobante = {(int)TipoComprobante.ManualClientes}");
        ...
        public async Task New(FacturacionManualViewModel model)
        {
            ...
            try
            {
                ...
                comprobante.IdTipoComprobante = (int)TipoComprobante.ManualClientes;
        ...
        public async Task Remove(int idComprobanteVta)
        {
            ...
            if (comprobante.IdTipoComprobante != (int)TipoComprobante.ManualClientes)
            {
                throw new BusinessException("El Tipo de Comprobante no es Manual. No se puede Eliminar.");
            }
        ...
        private void Validate(FacturacionManualViewModel comptobanteVta)
        {
        ...
        public async Task<int> GetContidadFacturasCompra()
        {
            var desde = new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1);
            var hasta = new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.DaysInMonth(DateTime.Now.Year, DateTime.Now.Month));

            var cantidad = await _comprobantesRepository.GetCantidadFacturasCompras(desde, hasta, _permissionsBusiness.Value.User.IdEmpresa);

            return cantidad;
        }
        public async Task<int> GetContidadFacturasVenta()
        {
            var desde = new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1);
            var hasta = new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.DaysInMonth(DateTime.Now.Year, DateTime.Now.Month));

            var cantidad = await _comprobantesRepository.GetCantidadFacturasVentas(desde, hasta, _permissionsBusiness.Value.User.IdEmpresa);

            return cantidad;
        }
        public async Task<decimal> GetIVADashboard()
        {
            var desde = new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1);
            var hasta = new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.DaysInMonth(DateTime.Now.Year, DateTime.Now.Month));

            var ivaCompras = await _comprobantesRepository.GetIVACompras(desde, hasta, _permissionsBusiness.Value.User.IdEmpresa);
            var ivaVentas = await _comprobantesRepository.GetIVAVentas(desde, hasta, _permissionsBusiness.Value.User.IdEmpresa);

            var result = ivaVentas - ivaCompras;
        ...
        public Task<int> GetCantidadFacturasComprasProveedor()
        {
            throw new NotImplementedException();
        }

        public Task<int> GetCantidadFacturasComprasPagasProveedor()
        {
            throw new NotImplementedException();
        }

        public Task<DataTablesResponse<FacturaCompra>> GetUltimasFacturasProveedor(DataTablesRequest request)
        {
            throw new NotImplementedException();
        }


        //public async Task<int> GetCantidadFacturasComprasProveedor()
        //{
        //    //var desde = new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1);
        //    //var hasta = new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.DaysInMonth(DateTime.Now.Year, DateTime.Now.Month));
        //    var cantidad = await _comprobantesRepository.GetCantidadFacturasComprasProveedor(_permissionsBusiness.Value.User.IdProveedor, _permissionsBusiness.Value.User.IdEmpresa);

        //    return cantidad;
        //}
        //public async Task<int> GetCantidadFacturasComprasPagasProveedor()
        //{
        //    var cantidad = await _comprobantesRepository.GetCantidadFacturasComprasPagasProveedor(_permissionsBusiness.Value.User.IdProveedor, _permissionsBusiness.Value.User.IdEmpresa);

        //    return cantidad;
        //}
        //public async Task<DataTablesResponse<Custom.Dashboard.FacturaCompra>> GetUltimasFacturasProveedor(DataTablesRequest request)
        //{
        //    var customQuery = _comprobantesRepository.GetUltimasFacturasProveedor(_permissionsBusiness.Value.User.IdProveedor, _permissionsBusiness.Value.User.IdEmpresa);
        //    var builder = _dbContext.Connection.QueryBuilder();

        //    return await _dataTablesService.Resolve<Custom.Dashboard.FacturaCompra>(request, customQuery.Sql, customQuery.Parameters, staticWhere: builder.Sql);
        //}
    }
}
