...
{
    public class SubdiarioComprasController : BusinessController
    {
        private readonly ISubdiarioComprasBusiness _subdiarioComprasBusiness;
        private readonly IMemoryCache _memoryCache;

        public SubdiarioComprasController(ISubdiarioComprasBusiness subdiarioComprasBusiness, IMemoryCache memoryCache)
        {
            _subdiarioComprasBusiness = subdiarioComprasBusiness;
        ...
        public async Task<ActionResult> Index()
        {
            ...
            var model = new SubdiarioComprasViewModel
            {
        ...
        {
            var results = await _subdiarioComprasBusiness.GetAll(request);
            return Json(results);
        }
        ...
        {
            var results = await _subdiarioComprasBusiness.GetDetalle(idComprobante, fechaDesde, fechaHasta);
            return Json(() => results);
        }
        ...
        public async Task<IActionResult> GenerarExcel(int idComprobante, DateTime? fechaDesde, DateTime? fechaHasta)
        {
            try
            {
                var excel = await _subdiarioComprasBusiness.GetExcel(idComprobante, fechaDesde, fechaHasta);
                if (excel != null)
                {
                    var fileGUID = Guid.NewGuid().ToString("N");
                    var fileName = $"SubdiarioCompras.xlsx";
        ...
        [HttpPost]
        [Authorize(Roles = "ADMIN, ADMINISTRACION, TESORERIA, CUENTAS POR PAGAR")]
        public async Task<ActionResult> DetalleImputaciones(DateTime? fechaDesde, DateTime? fechaHasta, int? idProveedor)
        {
            var results = await _subdiarioComprasBusiness.DetalleImputaciones(fechaDesde, fechaHasta, idProveedor);
            return Json(results);
        }
    }
}
