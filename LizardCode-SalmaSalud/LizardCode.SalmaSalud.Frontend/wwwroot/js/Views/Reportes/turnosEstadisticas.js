...
var TurnosEstadisticasView = new (function () {
...
function makeChartAusentes(data) {

    $("#chartAusentes").html('');

    if (!data) {
        $("#chartAusentes").html('<span class="pl-5">No se encontraron datos</span>');
        return;
    }
    ...
    var options = {
        ...
        series: [data.conAviso, data.sinAviso, data.cancelados],
        labels: ['C/Aviso', 'S/Aviso', 'Cancelados'],
        responsive: [{
            breakpoint: 1599,
            options: {
                chart: {
    ...
    chartA = new ApexCharts(
        document.querySelector("#chartAusentes"),
        options
    );
...
function makeChartTurnos(data) {
    ...
    if (!data || !data.length) {
        dataLabels.push('ESTADOS');
        dataSeries.push(0);
    }
    else {
        for (let i = 0; i < data.length; i++) {
            dataLabels.push(data[i].estado);
    ...
    chartE = new ApexCharts(
        document.querySelector("#chartTurnos"),
        options
    );
...
function renderCharts() {
    ...
    $.getJSON('/Reportes/ObtenerTurnosEstadisticasEstados', params)
        .done(function (data) {

            console.log(data);
            makeChartTurnos(data);
    ...
    $.getJSON('/Reportes/ObtenerTurnosEstadisticasAusentes', params)
        .done(function (data) {

            console.log(data);
            makeChartAusentes(data);

            Utils.modalClose();
        })
        .fail(function (data) {
            Utils.ShowError('Error generando gráfico A.');
        });
