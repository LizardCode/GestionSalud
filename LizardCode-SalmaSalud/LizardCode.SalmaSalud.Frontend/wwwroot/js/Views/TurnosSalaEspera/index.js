...
var TurnosView = new (function () {
    ...
    this.init = function () {
        ...
        modalSobreTurno = $('.modal.modalSobreTurno', mainClass);
        btSobreTurno = $('.toolbar-actions button.btSobreTurno', mainClass);
    ...
    function buildControls() {
        ...
        if ($('#hidLoggedUserTipo').val() == enums.TipoUsuario.Profesional)
            $('.btSobreTurno').show();

        if ($('#hidLoggedUserTipo').val() == enums.TipoUsuario.Recepcion)
            $('.btHistoriaClinicaTb').hide();

        modalSobreTurno.find('> .modal-dialog').addClass('modal-95');
        ...
        var columns = [
            ...
            { data: null, orderable: false, searchable: false, width: '10%', render: renderHoraTurno },
    ...
    function bindControlEvents() {
        ...
        btSobreTurno.on('click', sobreTurnoDialog);
    ...
    function renderAcciones(data, type, row, meta) {
        ...
        if (data.tipoTurno === 'DE')
            return btnCancelar + '&nbsp;' + btnAtender + (data.observaciones ? '&nbsp;' + btnObservaciones : '');
        else
            return btnCancelar + '&nbsp;' + btnReAgendar + '&nbsp;' + btnAtender + (data.observaciones ? '&nbsp;' + btnObservaciones : '');            
    }
    ...
    function sobreTurnoDialog() {

        window.location = '/Turnos/AgendaSobreTurnosPage';
    }       
