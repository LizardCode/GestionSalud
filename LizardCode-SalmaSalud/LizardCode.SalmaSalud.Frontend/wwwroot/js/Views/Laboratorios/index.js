...
var ABMLaboratoriosView = new (function () {
    ...
    function buildControls() {
        ...
        dtView = MaestroLayout.initializeDatatable('idProveedor', columns, order);

        $('input[name^=Provincia]').prop('readonly', true);
        $('input[name^=Localidad]').prop('readonly', true);
        $('input[name^=CodigoPostal]').prop('readonly', true);

        $('div[name^="RetencionAll"]').hide();
        $('div[name^="RetencionMonotributo"]').hide();
        $('div[name^="SinRetencion"]').hide();
    }
    ...
    function bindControlEvents() {
        ...
        $('select[name^="IdTipoIVA"]').on('change', function (e) {

            if ($(this).val() == enums.TiposIVA.Monotributo) {
                $('div[name^="RetencionAll"]').hide();
                $('div[name^="RetencionMonotributo"]').show();
                $('div[name^="SinRetencion"]').hide();
            }
            else if ($(this).val() == enums.TiposIVA.ResponsableInscripto) {
                $('div[name^="RetencionAll"]').show();
                $('div[name^="RetencionMonotributo"]').hide();
                $('div[name^="SinRetencion"]').hide();
            }
            else {
        ...
        $('.btnCUIT').on('click', function (e) {
            ...
            Ajax.Execute('/Proveedores/Padron/', params)
                .done(function (response) {
                    ...
                    geocoder.geocode({ 'address': address }, function (results, status) {
                        if (status === google.maps.GeocoderStatus.OK) {
                            ...
                            $('select[name^="IdTipoIVA"]').trigger('change');
                        }
                    });
        ...
        $('select[name^="IdCodigoRetencionGanancias"]').select2({ allowClear: true });
        $('select[name^="IdCodigoRetencionIVA"]').select2({ allowClear: true });
        $('select[name^="IdCodigoRetencionIBr"]').select2({ allowClear: true });
        $('select[name^="IdCodigoRetencionSUSS"]').select2({ allowClear: true });

    }

    function newDialogOpening(dialog, $form) {

        $('div[name^="RetencionAll"]', dialog).hide();
        $('div[name^="RetencionMonotributo"]', dialog).hide();
        $('div[name^="SinRetencion"]', dialog).show();
    ...
    function editDialogOpening($form, entity) {
        ...
        $form.find('.btnCUIT').prop('disabled', false);

        $form.find('#IdProveedor_Edit').val(entity.idProveedor);
        ...
        $form.find('#IdCodigoRetencionGanancias_Edit').select2('val', entity.idCodigoRetencionGanancias);
        $form.find('#IdCodigoRetencionIVA_Edit').select2('val', entity.idCodigoRetencionIVA);
        $form.find('#IdCodigoRetencionIBr_Edit').select2('val', entity.idCodigoRetencionIBr);
        $form.find('#IdCodigoRetencionSUSS_Edit').select2('val', entity.idCodigoRetencionSUSS);
        $form.find('#IdCodigoRetencionGananciasMonotributo_Edit').select2('val', entity.idCodigoRetencionGananciasMonotributo);
        $form.find('#IdCodigoRetencionIVAMonotributo_Edit').select2('val', entity.idCodigoRetencionIVAMonotributo);
        ...
        if (entity.idTipoIVA == enums.TiposIVA.Monotributo) {
            $('div[name^="RetencionAll"]').hide();
            $('div[name^="RetencionMonotributo"]').show();
            $('div[name^="SinRetencio"]').hide();
        }
        else if (entity.idTipoIVA == enums.TiposIVA.ResponsableInscripto) {
            $('div[name^="RetencionAll"]').show();
            $('div[name^="RetencionMonotributo"]').hide();
            $('div[name^="SinRetencio"]').hide();
        }
            ...
            $('div[name^="RetencionAll"]').hide();
            $('div[name^="RetencionMonotributo"]').hide();
            $('div[name^="SinRetencio"]').show();
        }

        if (entity.servicios.length > 0)
            $form.find('.dvServicios .repeater-component').Repeater()
                .source(entity.servicios);
        else
            $form.find('.dvServicios .repeater-component').Repeater()
                .clear();
    }
