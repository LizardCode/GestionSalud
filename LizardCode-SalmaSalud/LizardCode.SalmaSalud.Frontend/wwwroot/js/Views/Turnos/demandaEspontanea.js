...
var DemandaEspontaneaView = new (function () {
    var self = this;
    ...
    function buildControls() {
        ...
        Utils.rebuildValidator($('.frmDemandaEspontanea'), ':hidden:not(.validate):not(.validate :hidden), .no-validate');
    }

    function bindControlEvents() {

        $('.especialidadDemandaEspontanea').select2();
        $('.profesionalDemandaEspontanea').select2();
        $('.consultorioDemandaEspontanea').select2();

        $('.btSave')
            .on('click', function () {
                asignar($('.frmDemandaEspontanea'));
            });
        ...
        $('.frmDemandaEspontanea').on('click', '.fly-remove', function () {
            ...
            if (!response.afiliadoValido) {
                ...
                Modals.loadAnyModal('validacionPadronView', 'modal-70', action,
                    function () {
                        ...
                        $('.hdnDemandaForzarParticular').val(VALIDACION_FORZAR_PARTICULAR);
                        $('.hdnDemandaForzarPadron').val(VALIDACION_FORZAR_PADRON);
                        if (VALIDACION_FINANCIADOR_NRO)
                            $('.financiadorNro').val(VALIDACION_FINANCIADOR_NRO);

                        if (VALIDACION_CONTINUAR) {

                            Utils.modalQuestion("Demanda Espontánea", "¿Confirma el ingreso del paciente bajo la modalidad Demanda Espontánea?.",
                                function (confirm) {
                                    if (confirm) {
                                        Utils.modalLoader();
                                        $form.submit();
    ...
    this.ajaxDemandaEspontaneaBegin = function (context, arguments) {

        //Utils.modalLoader();
    }

    this.ajaxDemandaEspontaneaSuccess = function (context) {
        Utils.modalClose();

        //$('.demandaEspontaneaDialog').modal('hide');

        Utils.modalInfo('Demanda Espontánea', 'Se ha ingresado el paciente de forma correcta.', 5000);
        setTimeout(function () { window.location = '/Turnos' }, 1000);
    }

    this.ajaxDemandaEspontaneaFailure = function (context) {
        Utils.modalClose();
...
$(function () {
    DemandaEspontaneaView.init();
});

//# sourceURL=demandaEspontanea.js