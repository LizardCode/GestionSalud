var LoginView = new (function () {
                ...
                if (data.status == enums.AjaxStatus.OK)
                    ...
                    switch (data.detail.code) {
                        case -2:
                            ...
                            alertError(data.detail.message);
                            break;
                        case -1:
                            alertInfo("Código enviado");
                            $('#Pass').val('');
                            verIngresar(data.detail.message);
                            break;
                        case 0:
                            console.error(data.detail.message);
                            alertError('Se produjo un error grave', false);
                        ...
                        case 2: // Código vencido
                            //alertError(data.detail.message, 2000);
                            alertInfo('Código vencido. Solicite un nuevo código.');
                            //$('#RepeatPass').val('');
                            //$('#NewPass').val('').focus();
                            //requestNewPassword();
                            break;
    ...
    function bindEvents() {
        ...
        $('form').on('submit', function (e) {
            ...
            if (pass == '' && $('.btnVerSolicitar').is(':visible')) {
                e.preventDefault();
                $('#Pass').val('').focus();
                return false;
            }
        ...
        $('.btnVerSolicitar').on('click', function (e) {
            verSolicitar();
        });

        $('.btnVerIngresar').on('click', function (e) {
            verIngresar();
        });
    }

    function verSolicitar() {

        $('#password-field').val('');
        $('#password-field').hide();
        $('.btnVerIngresar').show();
        $('.btnVerSolicitar').hide();
        $('.btSubmit').text('Solicitar');

        $('form').attr('action', '/PortalPacientes/RequestAccessCode')
    }

    function verIngresar(accessCode) {

        $('#password-field').show();
        $('.btnVerIngresar').hide();
        $('.btnVerSolicitar').show();
        $('.btSubmit').text('Ingresar');

        if (accessCode)
            $('#Pass').val(accessCode);

        $('form').attr('action', '/PortalPacientes/SignIn')
    }
