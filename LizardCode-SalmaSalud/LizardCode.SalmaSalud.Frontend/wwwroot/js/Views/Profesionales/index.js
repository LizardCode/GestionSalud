...
var ABMProfesionalesView = new (function () {
    ...
    function buildControls() {
        ...
        var columns = [
            ...
            { data: 'nombre' },
            { data: 'especialidad' },
            { data: null, render: renderMatricula },
            { data: 'telefono' },
            { data: 'email' }
        ];
    ...
    function renderMatricula(data, type, row) {
        var sReturn = '';

        if (data.matricula) {
            sReturn = data.matricula + (data.matriculaProvincial ? ' - ' + data.matriculaProvincial : '');
        }
        else {
            sReturn = data.matriculaProvincial;
        }

        return sReturn.toLocaleUpperCase();
    }
    ...
    function bindControlEvents() {
        ...
        btAgenda.on('click', agendaDialog);
        ...
        $('.btnCUIT').on('click', function (e) {
            ...
            Ajax.Execute('/Profesionales/Padron/', params)
        ...
        $('input[name^=ExentoIIBB]').change(function () {
            var checked = $(this).is(':checked');

            $('.nroIBr').prop('disabled', checked);
        });

        $('[data-toggle="tooltip"]').tooltip();

        $(".firmaInput").change(function () {
            readURL(this);
        });

        $('.imgFirma-delete-icon').on('click', function (e) {
            removeFirma();
        });
    }
    ...
    function editDialogOpening($form, entity) {
        ...
        $form.find('#IdProfesional_Edit').val(entity.idProfesional);
        $form.find('#Matricula_Edit').val(entity.matricula);
        $form.find('#MatriculaProvincial_Edit').val(entity.matriculaProvincial);
        $form.find('#IdEspecialidad_Edit').select2('val', entity.idEspecialidad);
        $form.find('#Funcion_Edit').val(entity.funcion);
        $form.find('#Empresas_Edit').select2('val', entity.empresas);

        $form.find('#PolizaNumero_Edit').val(entity.polizaNumero);
        $form.find('#PolizaVencimiento_Edit').val(moment(entity.polizaVencimiento).format(enums.FormatoFecha.DefaultFormat));
        $form.find('#PolizaAseguradora_Edit').val(entity.polizaAseguradora);
        ...
        $form.find('#CantidadSobreturnos_Edit').val(entity.cantidadSobreturnos);
        $form.find('#TurnosIntervalo_Edit').val(entity.turnosIntervalo);

        if (!entity.nroIBr) {
            $('#ExentoIIBB_Edit').prop('checked', true).trigger('change');
        }

        if (entity.uploadedTipoFirma) {

            $('.imgFirma').attr('src', 'data:' + entity.uploadedTipoFirma + ';base64,' + entity.uploadedFirma);
            $('.firmaPreview').show();
        } else {
            removeFirma();
        } 
    ...
    function agendaDialog() {
        ...
        var idProfesional = selectedRows[0].idProfesional;
        //var action = '/Profesionales/AgendaView?idProfesional=' + idProfesional;
        //var widthClass = 'modal-95';

        //Modals.loadAnyModal('agendaDialog', widthClass, action, function () { ProfesionalesAgendaView.getAgenda(); }, function () { });

        window.location = '/Profesionales/AgendaPage?idProfesional=' + idProfesional;
    ...
    function readURL(input) {
        if (input.files && input.files[0]) {
            ...
            reader.onload = function (e) {
                $('.imgFirma').attr('src', e.target.result);
            }
