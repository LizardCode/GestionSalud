...
var ABMCargaAutomaticaView = new (function () {
    ...
    function bindControlEvents() {
        ...
        $('.btnCargaItems', modalNew).on('click', function (e) {
            ...
            var annoMesDesde = $('input[name^=AnnoMesDesde]', dialog).val().replaceAll('_', '');
            var annoMesHasta = $('input[name^=AnnoMesHasta]', dialog).val().replaceAll('_', '');
            var fecha = $('input[name^=Fecha]', dialog).val();
            var idProveedor = $('select[name^=IdProveedor]', dialog).val();
            ...
            if (annoMesDesde == "" || annoMesDesde == "/") {
                Utils.alertInfo("Ingrese el Mes/Año Desde parar cargar los Items de los Estimado a Facturar");
                return false;
            }
            if (annoMesHasta == "" || annoMesHasta == "/") {
                Utils.alertInfo("Ingrese el Mes/Año Hasta parar cargar los Items de los Estimado a Facturar");
                return false;
            }
            if (idProveedor == "") {
                Utils.alertInfo("Ingrese el Proveedor del Comprobante");
                return false;
            }
            if (idComprobante == "") {
            ...
            if (idMonedaEstimado == "") {
                Utils.alertInfo("Seleccione la Moneda del Estimado");
                return false;
            }
            ...
            var params = {
                annoMesDesde: annoMesDesde,
                annoMesHasta: annoMesHasta,
            ...
            Ajax.Execute('/CargaAutomatica/GetItemsEstimado/', params)
                .done(function (response) {

                    if (response.detail.length == 0) {
                        Utils.modalError("Comprobantes", "El Periodo seleccionado no Presenta Items pendientes de Recibir Comprobantes, Verifique.")
                        return false;
                    }
        ...
        $('select[name^=IdProveedor]', modalNew).on('change', function (e) {
            ...
            Ajax.Execute('/CargaAutomatica/GetComprobantesByProveedor/', params)
                .done(function (response) {
        ...
        $('select[name^=IdCondicion]', modalNew).on('change', function (e) {
            ...
            if ($(this).val() && $('input[name^=Fecha]', modalNew).val()) {
                ...
                Ajax.Execute('/CargaAutomatica/GetVencimiento/', params)
                    .done(function (response) {
    ...
    function retrieveRowDetails() {
        ...
        if (row.child.isShown()) {
            ...
            Ajax.Execute('/CargaAutomatica/Obtener/' + row.data().idComprobanteCompra, null, null, 'GET')
                .done(function (response) {
    ...
    function retrieveAsientoDetails() {
        ...
        if (row.child.isShown()) {
            ...
            Ajax.Execute('/CargaAutomatica/ObtenerAsiento/' + row.data().idComprobanteCompra, null, null, 'GET')
                .done(function (response) {
                    Ajax.ParseResponse(response,
                        function (data) {
    ...
    function retrieveValidarComprobante() {
        ...
        Ajax.Execute('/CargaAutomatica/ValidarComprobantesById/', params)
            .done(function (response) {
                Ajax.ParseResponse(response,
                    function (data) {
                        if (data.error)
    ...
    function calculaTotalesComprobante() {
        ...
        var impuestos = itemsComprobante.map(function (obj) { return { Alicuota: obj.Alicuota.value, Importe: 0 }; });
